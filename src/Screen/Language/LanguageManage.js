...
function LanguageManage() {
  ...
  const refecthListLanguagesCustom = () => {
    axios.get("http://localhost:8080/api/language/custom")
      .then(response => {
  ...
  const fetchLanguages = () => {
    Promise.all([
      ...
      .then(([res1, res2]) => {
        setAddLang(res1.data.find(item => item.code === 'MainAddLang')?.name || '');
        setListLang(res1.data.find(item => item.code === 'MainListLang')?.name || '');
        setMenuTitle(res1.data.find(item => item.code === 'MainTitle')?.name || '');
  ...
  const handleLanguageChange = (lang) => {
    setLanguage(lang);
    axios.put(`http://localhost:8080/api/choice`,
      lang,
      {
        headers: {
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Content style={{ padding: '0 50px', marginTop: 64 }}>
        <div style={{ padding: 24, minHeight: 280 }}>
          ...
          <h2>{ListLang}</h2>
          <LanguageList listLanguages={languages} onSuccess={handleAddLanguage} listCustom={languageCustom} />
        </div>
      </Content>
    </Layout>
  );
