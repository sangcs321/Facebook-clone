...
export const LanguageList: React.FC<LanguageListProps> = ({
  ...
  const fetchLanguages = () => {
    axios
      .get("http://localhost:8080/api/language/search?keyword=ListLang")
      .then((response) => {
        setTab1(
          response.data.find((item) => item.code === "ListLangTabVi")?.name ||
            ""
        );
        setTab2(
          response.data.find((item) => item.code === "ListLangTabEn")?.name ||
            ""
        );
        setColumn1(
          response.data.find((item) => item.code === "ListLangColumn1")?.name ||
            ""
        );
        setColumn2Vi(
          response.data.find((item) => item.code === "ListLangColumn2Vi")
            ?.name || ""
        );
        setColumn2En(
          response.data.find((item) => item.code === "ListLangColumn2En")
            ?.name || ""
        );
        setColumn3(
          response.data.find((item) => item.code === "ListLangColumn3")?.name ||
            ""
        );
        setAction1(
          response.data.find((item) => item.code === "ListLangAction1")?.name ||
            ""
        );
        setAction2(
          response.data.find((item) => item.code === "ListLangAction2")?.name ||
            ""
        );
        setUpdateLanguage(
          response.data.find((item) => item.code === "ListLangModalTitle")
            ?.name || ""
        );
      })
      .catch((error) => {
  ...
  const handleDelete = (record) => {
    console.log(
      "Xóa ngôn ngữ:",
      `http://localhost:8080/api/language/${record.code}`
    );
    Modal.confirm({
      title: "Xác nhận xóa",
      ...
      onOk: () => {
        return axios
          .delete(`http://localhost:8080/api/language/${record.code}`)
          .then(() => {
  ...
  const handleDeleteCustom = (record) => {
    Modal.confirm({
      ...
      onOk: () => {
        return axios
          .delete(`http://localhost:8080/api/custom`, {
            data: record,
          })
          .then(() => {
  ...
  const actionColumn = {
    ...
    render: (text, record) => (
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <Button type="primary" onClick={() => handleCustom(record)}>
          <FormatPainterOutlined />
        </Button>
  ...
  const actionColumnCus = {
    ...
    render: (text, record) => (
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <Button danger onClick={() => handleDeleteCustom(record)}>
          <DeleteOutlined />
        </Button>
      </div>
    ),
