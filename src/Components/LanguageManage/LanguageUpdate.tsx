...
const LanguageUpdate = ({ data, onSuccess }) => {
    ...
    const onFinish = (values) => {
        axios.put(`http://localhost:8080/api/language`, values)
            .then(response => {
    ...
    return (
        <Form form={form} name="languageForm" onFinish={onFinish} layout="inline">
            <Form.Item name="code" rules={[{ required: true, message: 'Vui lòng nhập mã ngôn ngữ!' }]}>
                <Input placeholder="Mã ngôn ngữ" value={data?.code} />
            </Form.Item>
            <Form.Item name="nameVi" rules={[{ required: true, message: 'Vui lòng nhập tên đối tượng!' }]}>
